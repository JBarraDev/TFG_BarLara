...
try {
	...
	// Organizar los productos por categorías para mostrarlos agrupados
	foreach ($results as $row) {
		$categoriaId = $row['categoria_id'];
		$itemId = $row['id'];
		
		// Si es la primera vez que vemos esta categoría, crear el array
		...
		// Si es la primera vez que vemos este producto, añadirlo al array
		if (!isset($menuItemsByCategory[$categoriaId]['items'][$itemId])) {
			// Obtener las valoraciones de este producto
		...
		// Si el producto tiene alérgenos, añadirlos
		...
		<div class="container">
			<?php if ($databaseConnectionError): ?>
				<div class="alert alert-danger" role="alert">
					Error de conexión: <?php echo htmlspecialchars($databaseConnectionError, ENT_QUOTES, 'UTF-8'); ?>
				</div>
			<?php else: ?>
					...
					<?php foreach ($menuItemsByCategory as $categoriaId => $categoria): ?>
						...
						<div class="mb-4">
							<button class="btn btn-link w-100 text-start text-decoration-none p-0 mb-3" 
									...
									data-bs-target="#categoria-<?php echo htmlspecialchars($categoriaId, ENT_QUOTES, 'UTF-8'); ?>" 
									aria-expanded="true" 
									aria-controls="categoria-<?php echo htmlspecialchars($categoriaId, ENT_QUOTES, 'UTF-8'); ?>">
								<h2 class="mb-0 text-primary border-bottom pb-2 d-flex justify-content-between align-items-center">
									<span><?php echo htmlspecialchars($categoria['nombre'], ENT_QUOTES, 'UTF-8'); ?></span>
							...
							<div class="collapse show" id="categoria-<?php echo htmlspecialchars($categoriaId, ENT_QUOTES, 'UTF-8'); ?>">
								<div class="row g-4">
										...
										<div class="col-md-6 col-lg-4">
											<div class="card h-100 shadow-sm border-0 card-item">
												...
												<?php if ($item['imagen_url']): ?>
													<img src="<?php echo htmlspecialchars($item['imagen_url'], ENT_QUOTES, 'UTF-8'); ?>" 
												...
												<div class="card-body d-flex flex-column">
														...
														<div class="mb-3">
															<div class="d-flex flex-wrap gap-2">
																<?php foreach ($item['alergenos'] as $alergeno): ?>
																	<img src="<?php echo htmlspecialchars($alergeno['icono_url'], ENT_QUOTES, 'UTF-8'); ?>" 
																		 alt="<?php echo htmlspecialchars($alergeno['nombre'], ENT_QUOTES, 'UTF-8'); ?>" 
																		 title="<?php echo htmlspecialchars($alergeno['nombre'], ENT_QUOTES, 'UTF-8'); ?>"
													...
													<div class="mb-3">
														<?php if ($item['valoracion_promedio'] !== null): ?>
															<div class="d-flex align-items-center gap-2">
																<div class="valoracion-estrellas" data-valoracion="<?php echo htmlspecialchars($item['valoracion_promedio'], ENT_QUOTES, 'UTF-8'); ?>">
																	<?php 
														...
														<?php if ($isClienteLoggedIn): ?>
															<button type="button" 
																	...
																	data-item-id="<?php echo htmlspecialchars($item['id'], ENT_QUOTES, 'UTF-8'); ?>"
																	data-item-nombre="<?php echo htmlspecialchars($item['nombre'], ENT_QUOTES, 'UTF-8'); ?>"
																	data-valoracion-actual="<?php echo $item['valoracion_usuario'] ? htmlspecialchars($item['valoracion_usuario']['valoracion'], ENT_QUOTES, 'UTF-8') : ''; ?>"
																	data-comentario-actual="<?php echo $item['valoracion_usuario'] ? htmlspecialchars($item['valoracion_usuario']['comentario'] ?? '', ENT_QUOTES, 'UTF-8') : ''; ?>">
													...
													<div class="mt-auto pt-3 border-top">
														...
														<!-- Botón Pedir -->
														<button type="button" class="btn btn-primary w-100 btn-pedir" data-item-id="<?php echo htmlspecialchars($item['id'], ENT_QUOTES, 'UTF-8'); ?>">
															Pedir
														</button>
													</div>
												</div>
